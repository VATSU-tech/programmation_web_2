...
try {
    ...
    if (!$comment || (int) $comment['user_id'] !== (int) $_SESSION['user_id']) {
        set_flash('error', 'Vous ne pouvez pas modifier ce commentaire.');
        redirect_back();
    }

    $stmt = $connection->prepare('UPDATE comments SET contenue = ? WHERE id = ?');
    $stmt->execute([$contenue, $comment_id]);

    set_flash('success', 'Commentaire modifié.');
} catch (Exception $e) {
